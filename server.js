...
async function handleGenerate(res, slug, req) {
	...
	if (!cfg.promptFile || !cfg.lessonId) {
		return json(res, 400, {
			error:
				"Exercise does not support LLM generation (no promptFile or lessonId)",
		});
	}
	...
	if (!grammarPoint) {
		return json(res, 400, {
			error: `Grammar index ${idx} not found in lesson ${cfg.lessonId}`,
		});
	}
	...
	if (!fs.existsSync(promptPath)) {
		return json(res, 404, {
			error: `Prompt file not found: ${cfg.promptFile}`,
		});
	}
	const promptTemplate = fs.readFileSync(promptPath, "utf8");

	// 7. Generate via LLM
	console.log(`\n  ⚡ Generate request: ${slug} (count: ${count})\n`);
	try {
		...
		console.log(
			`  ✓ Generation complete (${JSON.stringify(result).length} bytes)\n`,
		);
		json(res, 200, result);
	} catch (err) {
		console.error("  ✗ Generate error:", err.message);
		if (err.rawContent)
			console.error("  Raw content:", err.rawContent.slice(0, 300));
...
function route(req, res) {
	...
	if (p === "/api/exercises" && req.method === "GET")
		return handleListExercises(res);
	const genMatch = p.match(/^\/api\/generate\/([^/]+)$/);
	if (genMatch && req.method === "POST")
		return handleGenerate(res, genMatch[1], req);
